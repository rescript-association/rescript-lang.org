...
The cool thing is that you can create union types by simply putting them together:
...
let classify = (enc) =>
  ...
  | #ascii => "ascii" // string literals belog to ascii
  };
```
...
Here we only cover some basic usage of string literal types. 
