// Extracted from BuckleScript's ocaml/utils/warnings.ml
...
let getDescription = (num: int): option(string) => {
  numeric
  ->Js.Array2.find(((n, _)) => num === n)
  ->Belt.Option.map(((_, desc)) => desc);
};
...
let fuzzyLookup = (str: string): array((string, string)) => {
  let letters =
    Belt.Array.keep(letterDescriptions, ((l, _)) =>
      l->Js.String2.startsWith(str)
    );

  let numbers =
    numeric
    ->Belt.Array.keep(((n, _)) =>
        Belt.Int.toString(n)->Js.String2.startsWith(str)
      )
...
module Parser = {
  ...
  let parseExn = (input: string): array(token) => {
    ...
    while (pos^ <= last) {
      let cur = Js.String2.get(input, pos^);
      let newState =
        switch (state^) {
        | ParseModifier =>
          if (cur === "+" || cur === "-") {
  ...
  let merge = (base: array(token), other: array(token)) => {
    let dict =
      Js.Array2.copy(base)
      ->Belt.Array.map(token => (token.flag, token))
      ->Js.Dict.fromArray;

    Belt.Array.forEach(other, token => {
      dict->Js.Dict.set(token.flag, token)
    });
    ...
    ->Js.Array2.sortInPlaceWith((t1, t2) => {
        ...
        | (false, false)
        | (true, true) => Js.String2.localeCompare(f1, f2)->Belt.Float.toInt
        | (true, false) => (-1)
        | (false, true) => 1
        };
      });
