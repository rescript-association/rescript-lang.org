...
let make = () => {
  ...
  let clearInput = () => {
    React.Ref.current(inputRef)
    ->Js.Nullable.toOption
    ->Belt.Option.forEach(el => el->value(""));
  };
  ...
  <div
    className={
      activeClass
      ++ " bg-night hover:text-white hover:cursor-pointer flex justify-center p-2 px-3 rounded"
    }
    onClick>
    <input
      ...
      onKeyDown
      className={
        "transition-all ease-in-out duration-150 text-white bg-night border-none focus:outline-none "
        ++ activeInput
      }
    />
...
module Textbox = {
  ...
  let make = (~id: string) => {
    ...
    <div
      className={
        activeClass
        ++ " hover:text-white hover:cursor-pointer bg-night flex items-center p-2 px-3 rounded"
      }
      onClick>
