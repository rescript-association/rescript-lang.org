...
A lazy value of represents a deferred computation which will automatically memoize the result on the first run, and then return the memoized result on any repeated execution.

This is useful for e.g. defining functions and expressions for complex procedures that always return the same value, for example:
...
let lazyGetFiles = lazy(getFiles());
```
