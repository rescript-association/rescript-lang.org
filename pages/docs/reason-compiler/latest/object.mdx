...
Then use our [`Js.Dict`](/apis/latest/js/dict) (for "dictionary") API to bind to that JS object! In this mode, you can do all the metaprogramming you're used to with JS objects: get all keys through `Js.Dict.keys`, get values through `Js.Dict.values`, etc.
...
- You will still be required to transform variants and lists, so for seamless
  ...
  types](shared-data-types.md), such as array, string, int, float, etc.
...
Alternatively, you can use the [Pipe](pipe.md) feature in a later section for a nicer-looking access syntax:
