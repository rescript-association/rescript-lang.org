...
- Interpolated string: from `{j|hello ${name}|j}` to `` j`hello ${name}` `` (_update_: now it also works without `j`, as the type-safe version).
- Polymorphic variants: from `` `red`` to `#red`.
- Arrays: from `[|1,2,3|]` to `[1,2,3]`. In JS, arrays are the right default.
- Lists: from `[1,2,3]` to `list[1,2,3]` (_update_: now it is `list{1, 2, 3}`). This ties with upcoming plans to access containers in a uniform way: `set[...]` and `map[...]`. Maybe temporary.
