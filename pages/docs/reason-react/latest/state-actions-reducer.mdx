...
- A "**reducer**"! This [pattern-matches](/docs/manual/latest/pattern-matching-destructuring) on the possible actions and specifies what state update each action corresponds to. _In state machine terminology, this'd be a "state transition"_.
...
**Note**: just like for `self.handle`, sometimes you might be forwarding `send` to some helper functions. Pass the whole `self` instead and **annotate it**. This avoids a complex `self` record type behavior. See [Record Field `send`/`handle` Not Found](faq#record-field-send--handle-not-found).
