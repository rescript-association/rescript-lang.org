...
No surprise here. Since Reason's JSX has [punning syntax](/docs/manual/latest/jsx#Punning), that `button` will format into `<button onClick />`.
...
**Note 2**: sometimes you might be forwarding `handle` to some helper functions. Pass the whole `self` instead and **annotate it**. This avoids a complex `self` record type behavior. See [Record Field `send`/`handle` Not Found](faq#Record%20Field%20send%20/%20handle%20Not%20Found).
...
Sometimes, the component you're calling is from JavaScript (using the [ReasonReact<->ReactJS interop](interop)), and its callback prop asks you to pass a callback that receives more than one argument. In ReactJS, it'd look like:
