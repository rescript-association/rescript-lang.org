...
You don't need to use the `[@react.component]` declaration to write components. Instead you can write a pair of `foo` and `fooProps` functions such that `type fooProps: 'a => props and foo: props => React.element` and these will always work as React components! This works with your own version of [`[@bs.obj]`](/docs/reason-compiler/latest/object-2#function), [`[bs.deriving abstract]`](/docs/reason-compiler/latest/object#abstract-record-mode), or any other function that takes named args and returns a single props structure.
...
**Note on `default`:** to understand what `default` means, see [the ReScript docs](/docs/manual/latest/import-from-export-to-js#import-a-javascript-module-itself-es6-module-format).
