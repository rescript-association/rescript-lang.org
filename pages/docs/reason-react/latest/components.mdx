...
It has added a new function with `Props` as a suffix which uses [`[@bs.obj]`](/docs/reason-compiler/latest/object-2#Function) to create your props object. This function gets compiled away by BuckleScript and will be replaced by object literals when used.
...
You don't need to use the `[@react.component]` declaration to write components. Instead you can write a pair of `foo` and `fooProps` functions such that `type fooProps: 'a => props and foo: props => React.element` and these will always work as React components! This works with your own version of [`[@bs.obj]`](/docs/reason-compiler/latest/object-2#Function), [`[bs.deriving abstract]`](/docs/reason-compiler/latest/object#Abstract%20Record%20Mode), or any other function that takes named args and returns a single props structure.
...
**Note on `default`:** to understand what `default` means, see [the BuckleScript docs](/docs/reason-compiler/latest/import-export#Import%20an%20ES6%20Default%20Value).
