...
- `ReasonReactRouter.useUrl(~serverUrl)`: only usable in the new [function component api](components).
...
Notice that this is just normal [pattern matching](/docs/manual/latest/pattern-matching). You can combine it with other features, such as tuple + ReasonReact features like [subscriptions](subscriptions-helper.md) and reducer:
