...
Reason comes with the [JSX](/docs/manual/latest/jsx) syntax! ReasonReact has two different transforms that you can use for your components. This doc covers Version 3 of the transform which is very similar to how [the ReactJS JSX transform](https://reactjs.org/docs/introducing-jsx.html) works. To use it, put `{"reason": {"react-jsx": 3}` in your [`bsconfig.json`](/docs/manual/latest/build-configuration#reason,%20refmt) (schema [here](http://bucklescript.github.io/bucklescript/docson/#build-schema.json)).
...
By adding `{"reason": {"react-jsx": 3}` in your [`bsconfig.json`](/docs/manual/latest/build-configuration#reason-refmt).
