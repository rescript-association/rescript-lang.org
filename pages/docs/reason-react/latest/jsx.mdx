...
Reason comes with the [JSX](/docs/manual/latest/jsx) syntax! ReasonReact has two different transforms that you can use for your components. This doc covers Version 3 of the transform which is very similar to how [the ReactJS JSX transform](https://reactjs.org/docs/introducing-jsx.html) works. To use it, put `{"reason": {"react-jsx": 3}` in your [`bsconfig.json`](/docs/reason-compiler/latest/build-configuration#reason,%20refmt) (schema [here](http://bucklescript.github.io/bucklescript/docson/#build-schema.json)).
...
For help to migrate from version 2 to version 3, check the section [below](/docs/reason-react/latest/jsx#Migrating%20from%20version%202%20to%20version%203).
...
By adding `{"reason": {"react-jsx": 3}` in your [`bsconfig.json`](/docs/reason-compiler/latest/build-configuration#reason,%20refmt).
