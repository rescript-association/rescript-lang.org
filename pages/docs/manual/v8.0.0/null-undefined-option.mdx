...
To solve this, we provide access to more elaborate `null` and `undefined` helpers through the [`Js.Nullable`](/apis/latest/js/nullable) module. This somewhat works like an `option` type, but is different from it.
