...
The above code refers to the `message` binding in the file `Student.re`. Every ReScript file is also a module, so accessing another file's content is the same as accessing another module's content!
