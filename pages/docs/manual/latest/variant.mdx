...
- An int: `@as(5)`
- A float constant: `@as(1.5)`
- True/false: `@as(true)` and `@as(false)`
- Null: `@as(null)`

## Untagged variants

With _untagged variants_ it is possible to mix types together that normally can't be mixed in the ReScript type system. For example, with untagged variants you can represent a heterogenous array:
