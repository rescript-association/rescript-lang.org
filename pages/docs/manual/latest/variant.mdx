...
Behold, the crown jewel of ReScript data structures!
...
Along with a variant comes one of the most important features of ReScript, the `switch` expression.

A ReScript `switch` is visually similar to other languages' `switch` (aka a large `if/elseif/elseif...`). It allows you to check every possible case of a variant. To use it, enumerate every variant constructor of the particular variant you'd like to use, each followed by an `=>` and the expression corresponding to that case.
...
let greeting =
  switch myAccount {
  | None => "Hi!"
  | Facebook(name, age) => "Hi " ++ name ++ ", you're " ++ Js.Int.toString(age) ++ "-year-old."
...
The [standard library](//reasonml.github.io/api/index.html) exposes an important variant you'll come to hear a lot:

```res
type option<'a> = None | Some('a)
```

This is the convention used to simulate a "nullable" (aka `undefined` or `null`) value in other languages. Thanks to this convenience type definition, ReScript can default every value to be non-nullable. An `int` will always be an int, never "`int` **or** `null` **or** `undefined`". If you do want to express a "nullable int", you'd use `option<int>`, whose possible values are `None` or `Some(int)`. `switch` forces you to handle both cases; therefore, **a pure ReScript program doesn't have null errors**.
...
If you come from an untyped language, you might be tempted to try `type foo = int | string`. This isn't possible in ReScript; you'd have to give each branch a constructor: `type foo = Int(int) | String(string)`. Though usually, needing this might be an anti-pattern. The Design Decisions section below explains more.
...
ReScript also provides [a few other ways](/docs/reason-compiler/latest/function#modeling-polymorphic-function) to do this.
