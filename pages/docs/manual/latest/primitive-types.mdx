...
A regular expression like the above has the type `Js.Re.t`. The [Js.Re](/apis/javascript/latest/js/re) module contains the regular expression helpers you have seen in JS.
...
32-bits, truncated when necessary. We provides the usual operations on them: `+`, `-`, `*`, `/`, etc. See [Js.Int](/apis/javascript/latest/js/int) for helper functions.
...
Float requires other operators: `+.`, `-.`, `*.`, `/.`, etc. Like `0.5 +. 0.6`. See [Js.Float](/apis/javascript/latest/js/float) for helper functions.
