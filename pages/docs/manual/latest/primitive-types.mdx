...
Image a simplified `Console.log` function that prints a message to the console. Its type signature is `string => unit` and you'd use it like this `Console.log("something happened!")`. It takes a string as input, prints it, and then returns essentially nothing useful. When `unit` is the output of a function it means the function performs some kind of side-effect. 
