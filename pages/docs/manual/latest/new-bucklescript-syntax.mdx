...
- Polymorphic variants: from `` `foo`` to `#foo`.
...
- Exception: from `try (foo) { | Bar => 1}` to `try foo catch { | Bar => 1}`
- No custom infix operator for now (including `mod`).
- Object access: from `foo##bar #= baz` to `foo["bar"] = baz`. Rejoice!
- Object: from `Js.t({"a": b})` to just `{"a": b}`. The `Js.t` part is now implicit.
- Attribute: from `[@foo bar]` to `@foo(bar)`. From `[%foo bar]` to `%foo(bar)`.
- Removed dereference syntax `foo^`. Just use `foo.contents`.
...
Create a file with the `.res` or `.resi` file extension to enable the new syntax parser (no further configuration required).
...
| `let x = 5; x = x + 1;` | `let x = ref(5); x := x.content + 1` |
...
| `a ? b : c`           | Same                                                                |
| `switch`              | `switch` but [super-powered pattern matching!](pattern-matching.md) |
