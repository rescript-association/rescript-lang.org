...
This looks much worse than the JS counterpart! Now we need to read the actual logic inside-out. We also cannot use the [`|>` (pipe last)](./pipe-last) operator here, since the object comes _first_ in the binding. But `->` works!
