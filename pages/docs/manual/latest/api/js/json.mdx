...
Serializes a ReScript record into to a JSON compliant string.

Raises a `Js.Exn.Error` exception if the record cannot be serialized.

```res
type person = {name: string}
let p: person = {name: "Albert"}
let s: string = Js.Json.serializeExn(p)
```

> **Note:** This function may change or be removed in a future version of ReScript.
...
Deserializes a JSON string created by `serializeExn()` into a ReScript record.

Raises a `Js.Exn.Error` exception if the string cannot be deserialized.

```res
type person = {name: string}
let p: person = Js.Json.deserializeUnsafe(`{ "name": "Albert" }`)
```
