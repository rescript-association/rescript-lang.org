...
Deserializes a JSON string without any type checks.

Raises a `Js.Exn.Error` exception if the JSON string parsing fails.
...
Serializes any type to a JSON string.

Raises a `Js.Exn.Error` exception if the value cannot be serialised.
