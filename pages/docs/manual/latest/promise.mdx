...
This type signature means that `make` takes a callback that takes 2 named arguments, `resolve` and `reject`. Both arguments are themselves [uncurried callbacks](/docs/reason-compiler/latest/function#Solution:%20Guaranteed%20Uncurrying) (they have a dot). `make` returns the created promise.
