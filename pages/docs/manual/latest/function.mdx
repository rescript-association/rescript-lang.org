...
A function always takes an argument; but sometimes, we'd use it for e.g. side-effects, and don't have anything to pass to it. In other languages, we'd conceptually pass "no argument". In ReScript, every function takes an argument; here we'd conventionally pass it the value `()`, called "unit".
...
`()` is a totally normal value, the single possible value in `unit`. ReScript gave it a special syntax out of convenience.
...
In imperative languages, including JavaScript, recursive functions are rarely used. In ReScript, recursive functions are common, and can typically replace an imperative loop.
...
Using recursion with a `switch` statement is a powerful combination that can sometimes produce code simpler than a standard library function like `reduce`.
...
When the recursion happens in the tail position (meaning that the recursive call is the last step performed in the function), this can be optimized in a way that is stack safe. Fortunately for us, the compiler is able to detect and perform this optimization.

For example, if you take our `listHas` function and compile it to JavaScript, you'll see that it removes the tail-calls by compiling into a `while` loop! ReScript gives us the best of both worlds.

For writing `for` or `while` loops instead of recursive functions, see: [Control Flow](control-flow.mdx).
