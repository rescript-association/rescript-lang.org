...
We discourage the usage of the `|>` / pipe-last operator. Using it may sometimes have a runtime cost due to currying. Please use the [pipe-first](./pipe-first) operator instead.

Pipe-last is mainly used in legacy codebases.
...
- Pipe-last is a **binding to a compiler intrinsic,** it will optimize some currying, but not all. Pipe-first (`->`) is implemented as syntactic sugar within ReScript and guarantees zero extra currying overhead.
