...
**Before you proceed**, please make sure that ReScript is what your team needs! As much as we vouch for ReScript and BuckleScript's popularity, please don't unnecessarily thrash your colleagues and give them a bad first impression. That's hard to undo afterward.

This guide covers a workflow that's helped us convert things over rapidly and efficiently. It's not intended to go over language/FFI features (though it puts them in context). Basic ReScript/BuckleScript knowledge is assumed.
...
- To communicate with external JS files, use `external`. They're our [foreign function interface](/docs/reason-compiler/latest/intro-to-external).
