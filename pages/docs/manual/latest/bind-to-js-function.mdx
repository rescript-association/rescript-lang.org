...
It is best practice to inspect data received from untrusted external functions to ensure it contains what you expect. This helps avoid run-time crashes and unexpected behavior. If you're certain about what an external function returns, simply assert the return value type as `string` or `array<int>` or whatever you want it to be. Otherwise use `unknown`. The ReScript type system will prevent you from using an `unknown` value until you first inspect it and "convert" it using JSON parsing utilities or similar tools.

Consider the following example. Here are two external functions that access the value of a property on a JavaScript object. The `getPropertyUnsafe` function has been defined as returning an `'a`, which means "anything you want it to be". ReScript allows you to use this value as a `string` or `int` or `array` or any other type. Quite convenient! But if the property is missing or contains something unexpected, your code might break. You can make the binding a bit more safe by changing the `'a` to a `string` or `option<'a>`, but this doesn't completely eliminate the problem.
