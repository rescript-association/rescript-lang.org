...
For a function whose argument is passed a JavaScript value that's potentially `null` or `undefined`, it's idiomatic to convert it to an `option`. The conversion is done through the helper functions in ReScript's [`Js.Nullable`](/apis/javascript/latest/js/nullable#t) module. In this case, `toOption`:
