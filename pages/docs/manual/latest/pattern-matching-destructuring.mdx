...
You can destructure Lists by pattern matching on list elements:

<CodeTab labels={["ReScript", "JS Output"]}>

```res example
let myList = list{"Hello", "Goodbye"}
let message = switch myList {
| list{} => "Empty"
| list{head, ...tail} => {
    head ++ " and " ++ tail->Belt.List.length->Belt.Int.toString ++ " more"
  }
}
```
...
Similarly, you can destructure Arrays by pattern matching on their elements.

Note that spread `...` is not supported for Array pattern matching.

<CodeTab labels={["ReScript", "JS Output"]}>

```res example
let myArr = [1, 2]
let (x, y) = switch myArr {
| [x] => (x, 0) // Exactly one element
