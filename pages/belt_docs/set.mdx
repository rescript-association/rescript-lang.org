...
```re example
Belt.Set.fromArray([|1, 3, 2, 4|], ~id=(module IntCmp))->Belt.Set.toArray; /* [|1, 2, 3, 4|] */
```
...
Checks if two sets are equal.<br/>
Returns true if `Belt.Set.toArray(s0) == Belt.Set.toArray(s1)`.
...
```re example
let s0 = Belt.Set.fromArray([|5,2,3,5,6|], ~id:(module IntCmp));
s0->Belt.List.reduce([], (element, acc) => acc->Bs.List.add(element)); /* [6,5,3,2] */
```
