...
(e.g. modifying an array in place using [Js.Array.push](/apis/latest/js/array#push)).

Therefore, when a correpsonding module is available in the [Belt](/apis/latest/belt) standard library,
it is recommended to use the Belt version rather than the Js version.
For example, you should prefer [Belt.Array](/apis/latest/belt/array)
to [Js.Array](/apis/latest/js/array)
and [Belt.Map.String](/apis/latest/belt/map-string)
to [Js.Dict](/apis/latest/js/dict).

## Argument Order

For historical reasons, some APIs in the Js namespace (e.g. [Js.String](/apis/latest/js/string)) are
using the data-last argument order whereas others (e.g. [Js.Date](/apis/latest/js/date)) are using data-first.
...
For some modules with data-last argument order (e.g. [Js.String](/apis/latest/js/string)),
there currently exists a matching module suffixed with "2" (e.g. [Js.String2](/apis/latest/js/string-2))
