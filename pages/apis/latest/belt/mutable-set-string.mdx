...
This module is [Belt.MutableSet](/apis/latest/belt/mutable-set) specialized with key type to be a `string` type.
It is more efficient in general, the API is the same with [Belt.MutableSet](/apis/latest/belt/mutable-set) except its key type is fixed, and identity is not needed (using the built-in one)
