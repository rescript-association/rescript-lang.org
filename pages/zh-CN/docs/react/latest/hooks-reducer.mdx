...
description: "关于 ReScript 中 useReducer React Hook 的细节"
...
[useState](./hooks-state) 的替代方案。它接收一个 `(state, action) => newState` 类型的 reducer，并返回当前 `state` 以及与其配套的 `dispatch` 函数 `(action) => unit`。
...
**注意**： 你会发现，归功于 [不可变记录](/docs/manual/latest/record)，[变体](/docs/manual/latest/variant) 和 [模式匹配](/docs/manual/latest/pattern-matching-destructuring) 特性，action / reducer 模式可以在 ReScript 中轻松地表达 action 和 state 变换。
